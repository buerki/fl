...
##############################################################################
# fl-density.sh 
copyright="(c) Cardiff University 2016, 2017, 2020, 2021; written by Andreas Buerki"
version='0.5.3'
####
...
for line in $(sed -e 's/_/UNDERSCORE/g' -e 's/	/_/g' -e "s/\'\'/_SYM/g" <<<"$pos"); do
	(( linecount += 1 ))
	# if we have a FS
	if [ "$(egrep 'UNDERSCOREUNDERSCORE' <<<"$line")" ]; then
...
$fl_line"
	# if we have a verb
	elif [ "$(egrep '_V[[:upper:]]+$' <<<"$line")" ]; then
			# if -1 option is active and we have a contracted verb
			if [ "$wc_option_1" ] && [ "$(egrep "$verb_contractions" <<<"$line")" ];then
	...
	# if we have a normal token
		...
		if [ "$wc_option_1" ];then
			...
			if [ "$(egrep "$word_contractions|$possessive_s" <<<"$line")" ]; then
				if [ "$detail" ]; then 
			...
			if [ "$(egrep "$possessive_s" <<<"$line")" ]; then
				if [ "$detail" ]; then
					if [ "$aux" ]; then sed $extended -e 's/_.+\$*$//' <<<"$line" >> $log_name
					else sed $extended -e 's/_.+\$*$//' <<<"$line" ;fi # cut off the tag
				fi
