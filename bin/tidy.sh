...
# tidy.sh
version='0.9.6'
copyright="(c) 2013-14 Andreas Buerki; 2015, 2020 Cardiff University; Licensed under the EUPL v. 1.2 or later"
####
# DESCRRIPTION:
...
do
	...
	if [ $consecutive_spaces -eq 1 ]; then
		if [ "$retain_sumoftok" ]; then
			...
			< $lists | grep -E -v '^[0-9]+$' | eval $korean sort $sort_by_n >> $output_filename
		else
			sed $extended -e "s/$separator([0-9]+)  /$separator	\1	/g" -e 's/ $//g' \
			< $lists | grep -E -v '^[0-9]+$' | eval $korean sort $sort_by_n > $output_filename
		fi
		...
		if [ "$retain_sumoftok" ]; then
			# first write sum of tokens to file
			grep -E '^[0-9]+$' $lists > $output_filename
			sed $extended -e "s/$separator([0-9]+) *$/$separator	\1/g" -e 's/ $//g' \
			< $lists | grep -E -v '^[0-9]+$' | eval $korean sort $sort_by_n >> $output_filename
		else
			sed $extended -e "s/$separator([0-9]+) *$/$separator	\1/g" -e 's/ $//g' \
