...
##############################################################################
# installer
copyright="Copyright (c) 2015-6, 2025 Cardiff University"
...
export linux_only=""
export osx_only=""
export licence="European Union Public Licence (EUPL) v. 1.2."
export URL="https://interoperable-europe.ec.europa.eu/collection/eupl/eupl-text-eupl-12"
# define functions
help ( ) {
	echo "
...
do
	...
	V)	echo "$(basename $(sed 's/ //g' <<<$0))	-	version $version"
		echo "$copyright"
		echo "licensed under the EUPL V.1.2"
...
if [ "$uninstall" ]; then
	...
	bash -lc 'if [ "$(grep -E -o "/usr/local/bin" <<<$PATH)" ]; then
...
for file in $components; do
	...
	# remove programme file in $HOME/bin or /usr/local/bin
	rm -f "$HOME/bin/$file" 2>/dev/null
	rm -f "usr/local/bin/$file" 2>/dev/null
	existing=""
done'
if [ "$DARWIN" ] && [ "$osx_only" ]; then
...
echo "$components $(if [ "$DARWIN" ]; then :;else echo "$linux_only placed in $HOME/.icons";fi)" | tr ' ' '\n'
echo ""
sleep 10
